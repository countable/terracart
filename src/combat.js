...
(function (root) {
  ...
  //
  // The two weapons now differ in KIND, not just tint:
  ...
  const SHOT = {
    ...
    staff: { speedCps: 3.2, rangeCells: 7, color: 0x9ad6ff, dotPx: 3, lenPx: 6, widthPx: 3,
             phaseMs: FIRE_INTERVAL_MS / 2, pierce: true, energyCost: 1 },
  };
  ...
  // melee-equivalent rate, SPLIT across the ranged slots, then weighted by the
  // slot (SHOT_DMG_MUL — the staff hits double, priced in energy). So a bow
  // alone delivers HALF its tier's melee rate, a staff alone delivers the full
  // rate, and the interval cancels out of the delivered rate entirely. An
  // empty slot fires nothing at all.
  //
  ...
  function shotDamage(relics, slot) {
    ...
    return Math.max(1, Math.round(perSecond * (SHOT_DMG_MUL[slot] || 1) * FIRE_INTERVAL_MS / 1000));
  }
