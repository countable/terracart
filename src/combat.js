...
// be in play, a shot carries its FULL tier-equivalent melee rate — the same
// rate a sword of that tier deals — with no split across ranged slots: there
// used to be one (bow and staff fired simultaneously and stacked, so their
// shares were priced to sum to one sword), but exclusivity already prevents
// the double-dip the split existed to fix.
//
...
(function (root) {
  ...
  const SHOT = {
    ...
    staff: { speedCps: 3.2, rangeCells: 7, color: 0x9ad6ff, lenPx: 6, widthPx: 3, phaseMs: 0 },
  };
  ...
  // fired once a second — see the "only one weapon fights" note at the top of
  // the file. An empty slot fires nothing at all.
  //
