...
test('cellsPerEdgeForLat: pinned equator value (2445.98/7 ≈ 349)', () => {
  const cpe = WorldGen.cellsPerEdgeForLat(0);
  assert.eq(cpe, 349, 'equator = 349 cells');
});
...
test('cellsPerEdgeForLat: at 60 lat is ~175 cells', () => {
  // tileEdgeMeters(60) = mpp(60,14)*256 = 4.777...*256 = 1222.99... / 7 = 174.7 -> round = 175
  const cpe = WorldGen.cellsPerEdgeForLat(60);
  assert.eq(cpe, 175, '60° lat = 175 cells');
});
